...
- **LATEST (5.9): Signature Verification** — `verifyEsignResponseHash()` verifies the returned PKCS#7 against the hash that was sent, reports the signer certificate, and checks revocation over OCSP with a CRL fallback. Works for hash-mode signing over any payload, not just PDFs
