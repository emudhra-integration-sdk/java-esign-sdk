...
import esign.text.ImgWMF;
import esign.text.Rectangle;
import esign.text.Version;
import esign.text.pdf.interfaces.PdfIsoConformance;
import esign.text.pdf.interfaces.PdfPageActions;
...
import esign.text.pdf.interfaces.PdfRunDirection;
import esign.text.pdf.interfaces.PdfViewerPreferences;
import esign.text.pdf.interfaces.PdfDocumentActions;
import esign.text.pdf.interfaces.PdfXConformance;
import esign.text.pdf.interfaces.PdfAnnotations;
import esign.text.*;
//import static com.itextpdf.text.DocWriter.getISOBytes;
import esign.text.error_messages.MessageLocalization;
import esign.text.io.TempFileCache;
import esign.text.log.Counter;
import esign.text.log.CounterFactory;
import esign.text.pdf.collection.PdfCollection;
import esign.text.pdf.events.PdfPageEventForwarder;
import esign.text.pdf.internal.PdfIsoKeys;
import esign.text.pdf.internal.PdfVersionImp;
import esign.text.pdf.internal.PdfXConformanceImp;
 ...
 * this Document will be written to the outputstream.</P>
 */
...
//            }
    ...
    static public class PdfTrailer extends PdfDictionary {
         ...
         * @param	info	an indirect reference to the info object of the PDF
         * document
         * @param encryption
         * @param fileID
         * @param prevxref
...
//	the PdfDocument instance
     ...
     *
     * @param ensureNewLine Tells whether a new line shall be enforced. This may
     * cause side effects for elements that do not terminate the lines they've
     * started because those lines will get terminated.
     ...
     * @since	2.1.6
     * @throws	DocumentException	if you try setting the leading after the
     * document was opened.
     */
 ...
 * to pages and other page trees.
     ...
     * @param order an array with the new page sequence. It must have the same
     ...
     *
     * @param	tabs	Can be PdfName.R, PdfName.C or PdfName.S. Since the Adobe
...
//	[C2] PdfVersion interface
    ...
    /**
     * @see
     * com.itextpdf.text.pdf.interfaces.PdfVersion#setAtLeastPdfVersion(char)
     */
    ...
    /**
     * @see
     * com.itextpdf.text.pdf.interfaces.PdfVersion#setPdfVersion(com.itextpdf.text.pdf.PdfName)
     */
    ...
    /**
     * @see
     * com.itextpdf.text.pdf.interfaces.PdfVersion#addDeveloperExtension(com.itextpdf.text.pdf.PdfDeveloperExtension)
     * @since	2.1.6
     */
...
//  [C3] PdfViewerPreferences interface
    ...
    /**
     * @see
     * com.itextpdf.text.pdf.interfaces.PdfViewerPreferences#setViewerPreferences(int)
     */
    ...
    /**
     * @see
     * com.itextpdf.text.pdf.interfaces.PdfViewerPreferences#addViewerPreference(com.itextpdf.text.pdf.PdfName,
...
//  [C5] named objects: named destinations, javascript, embedded files
     ...
     *
     * @param	map	a map with strings as keys for the names, and structured
     * strings as values for the destinations
     * @param	page_offset	number of pages that has to be added to the page
     * numbers in the destinations (useful if you use this method in combination
     * with PdfCopy).
     * @since	iText 5.0
     */
     ...
     * @param unicode select JavaScript unicode. Note that the internal Acrobat
     * JavaScript engine does not support unicode, so this may or may not work
     * for you
     */
     ...
     * @param fileStore an array with the file. If it's <CODE>null</CODE> the
     * file will be read from the disk
     ...
     * @throws IOException if the file attachment could not be added to the
     * document
     */
...
// [C6] Actions (open and additional)
    ...
    /**
     * @see
     * com.itextpdf.text.pdf.interfaces.PdfDocumentActions#setOpenAction(java.lang.String)
     */
    ...
    /**
     * @see
     * com.itextpdf.text.pdf.interfaces.PdfDocumentActions#setOpenAction(com.itextpdf.text.pdf.PdfAction)
     */
    ...
    /**
     * @see
     * com.itextpdf.text.pdf.interfaces.PdfDocumentActions#setAdditionalAction(com.itextpdf.text.pdf.PdfName,
...
//  [C8] AcroForm
    ...
    /**
     * @see
     * com.itextpdf.text.pdf.interfaces.PdfAnnotations#addAnnotation(com.itextpdf.text.pdf.PdfAnnotation)
     */
    ...
    /**
     * @see
     * com.itextpdf.text.pdf.interfaces.PdfAnnotations#addCalculationOrder(com.itextpdf.text.pdf.PdfFormField)
     */
...
//  [C9] Metadata
     ...
     * PdfDocument.
     *
     * @since 5.4.4 just creates XmpWriter instance which will be serialized in
     * close.
     */
...
//  [C10] PDFX Conformance
    ...
    /**
     * @see
     * com.itextpdf.text.pdf.interfaces.PdfXConformance#setPDFXConformance(int)
     */
    ...
    /**
     * @see
     * com.itextpdf.text.pdf.interfaces.PdfXConformance#getPDFXConformance()
     */
     ...
     *
     * @return <code>true</code> if the PDF has to be in conformance with any of
     * the PDF ISO specifications
     */
...
//  [C11] Output intents
     ...
     *
     * @param outputConditionIdentifier a value
     * @param outputCondition a value, "PDFA/A" to force GTS_PDFA1, otherwise
     * cued by pdfxConformance.
     * @param registryName a value
     * @param info a value
     ...
     * @param checkExistence <CODE>true</CODE> to just check for the existence
     * of a valid output intent dictionary, <CODE>false</CODE> to insert the
...
//  [F1] PdfEncryptionSettings interface
    ...
    /**
     * @deprecated As of iText 2.0.7, use {@link #ALLOW_MODIFY_CONTENTS}
     ...
     * PdfWriter#STANDARD_ENCRYPTION_40, PdfWriter#STANDARD_ENCRYPTION_128,
     * PdfWriter#ENCRYPTION_AES_128 or PdfWriter#ENCRYPTION_AES_256
     ...
     * @param ownerPassword the owner password. Can be null or empty
     * @param permissions the user permissions
     * @param strength128Bits <code>true</code> for 128 bit key length,
     * <code>false</code> for 40 bit key length
     * @throws DocumentException if the document is already open
     ...
     * and AllowDegradedPrinting. The permissions can be combined by ORing them.
     *
     * @param strength <code>true</code> for 128 bit key length,
     * <code>false</code> for 40 bit key length
     * @param userPassword the user password. Can be null or empty
     * @param ownerPassword the owner password. Can be null or empty
     * @param permissions the user permissions
...
//  [F4] adding (and releasing) form XObjects
     ...
     * @param forcedName the template name, rather than a generated one. Can be
     * null
...
//  [F12] tagged PDF
    ...
    /**
     * Fix structure of tagged document: remove unused objects, remove unused
     * items from class map, fix xref table due to removed objects.
     */
...
//  [F13] Optional Content Groups
    ...
    /**
     * @param erase true to erase the {@link PdfName#OCGS} and {@link PdfName#D}
...
//  [U1] page size
    ...
    /**
     * Returns the intersection between the crop, trim art or bleed box and the
     * parameter intersectingRectangle. This method returns null when - there is
     * no intersection - any of the above boxes are not defined - the parameter
     * intersectingRectangle is null
     *
     * @param boxName crop, trim, art, bleed
     * @param intersectingRectangle the rectangle that intersects the rectangle
     * associated to the boxName
    ...
    public Rectangle getBoxSize(final String boxName, final Rectangle intersectingRectangle) {
        ...
        esign.text.awt.geom.Rectangle boxRect = new esign.text.awt.geom.Rectangle(pdfRectangle);
        esign.text.awt.geom.Rectangle intRect = new esign.text.awt.geom.Rectangle(intersectingRectangle);
        esign.text.awt.geom.Rectangle outRect = boxRect.intersection(intRect);
...
//  [U3] page actions (open and close)
    ...
    /**
     * @see
     * com.itextpdf.text.pdf.interfaces.PdfPageActions#setPageAction(com.itextpdf.text.pdf.PdfName,
     * com.itextpdf.text.pdf.PdfAction)
     */
    ...
    /**
     * @see
     * com.itextpdf.text.pdf.interfaces.PdfPageActions#setTransition(com.itextpdf.text.pdf.PdfTransition)
     */
...
//  [M1] Color settings
     ...
     * <CODE>PdfName.DEFAULTGRAY</CODE>, <CODE>PdfName.DEFAULTRGB</CODE> or
     * <CODE>PdfName.DEFAULTCMYK</CODE>
     * @param cs the colorspace. A <CODE>null</CODE> or <CODE>PdfNull</CODE>
...
//  [M3] Images
     ...
     * resources. It is used with templates and
     * <CODE>Document.add(Image)</CODE>. Use this method only if you know what
     * you're doing!
     *
     ...
     * @param fixedRef the IndirectReference, may be null then a new indirect
     ...
     *
     * @param	content a byte array that may already been added to the writer
     * inside a stream object.
...
//  [F12] tagged PDF
     ...
     * work in all cases.
     *
     * @param rgbTransparencyBlending <code>true</code> to set the transparency
     * blending colorspace to RGB, <code>false</code> to use the default
     * blending colorspace
     * @since 2.1.0
     */
    ...
    /**
     * A wrapper around PdfAnnotation constructor. It is recommended to use this
     * wrapper instead of direct constructor as this is a convenient way to
     * override PdfAnnotation construction when needed.
     *
     * @param rect
     * @param subtype
     * @return
